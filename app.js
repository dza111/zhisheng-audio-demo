...
const presetLibrary=[
  {id:'rap',name:'智声说唱预设',description:'强化咬字、动态与低频控制，适配 Hip-Hop 制作。',icon:'audio-waveform',tag:'RAP / HIP-HOP',file:'智声说唱预设.trackpreset'},
  {id:'backing',name:'和声backing预设',description:'为和声与叠唱建立清晰、宽阔且可控的空间层次。',icon:'waves',tag:'BACKING VOCAL',file:'和声backing预设.trackpreset'},
  {id:'udg',name:'智声udg预设',description:'适合需要厚度与质感的人声起始处理，便于快速进入制作。',icon:'mic-vocal',tag:'ZHISHENG UDG',file:'智声udg预设.trackpreset'},
  {id:'pop',name:'智声流行预设',description:'保留人声细节，建立明亮、有空气感的流行表达。',icon:'sparkles',tag:'POP VOCAL',file:'智声流行预设.trackpreset'},
  {id:'retro',name:'智声复古预设',description:'加入温暖、柔和的复古色彩，适合怀旧与磁带感作品。',icon:'disc',tag:'RETRO COLOR',file:'智声复古预设.trackpreset'},
  {id:'electronic',name:'电音电子预设',description:'为电子人声与合成器声部建立更鲜明的动态和空间方向。',icon:'radio',tag:'ELECTRONIC',file:'电音电子预设.trackpreset'}
];
function presetDetail(id){ const preset=presetLibrary.find(item=>item.id===id); if(!preset)return notFound(); return `<main class="shell page preset-detail-page"><section class="preset-detail-hero"><div><div class="eyebrow">EFFECT PRESET / ${preset.tag}</div><h1>${preset.name}</h1><p>${preset.description}</p><div class="preset-detail-actions"><a class="btn btn-primary" href="/assets/presets/${encodeURIComponent(preset.file)}" download>${icon('download')} 下载预设文件</a>${link('/presets','返回效果预设','btn btn-secondary')}</div></div><div class="preset-detail-art preset-photo-art">${presetPhoto(preset.id)}</div></section><section class="preset-detail-grid"><article class="preset-audio-panel"><div class="eyebrow">AUDIO PREVIEW</div><h2>试听音频</h2><p class="audio-placeholder">试听文件待上传，收到对应音频后将在这里直接播放。</p><div class="audio-placeholder-bar"><span>${icon('play')}</span><div><strong>试听入口已预留</strong><small>支持 MP3 / WAV</small></div></div></article><aside class="preset-file-panel"><div class="eyebrow">PRESET FILE</div><h2>文件信息</h2><dl><div><dt>文件名</dt><dd>${preset.file}</dd></div><div><dt>类型</dt><dd>Studio One Track Preset</dd></div><div><dt>用途</dt><dd>${preset.tag}</dd></div></dl></aside></section></main>`; }
