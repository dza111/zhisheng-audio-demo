...
function personGenres(person){ const styles=person.styles.join(' '); const genres=[]; if(/流行|R&B|广告/.test(styles))genres.push('流行'); if(/说唱|Hip-Hop|Rap|Trap/.test(styles))genres.push('说唱'); if(/古典|民谣|国风|影视|配乐|乐队/.test(styles))genres.push('古典'); if(/影视|配乐/.test(styles))genres.push('影视配乐'); return genres.join(' '); }
...
function teachingPage(course){ const courses={recording:{name:'录音课程',intro:'从录音环境、话筒摆位到人声录制流程，建立可靠的录音基础。',icon:'mic-vocal',modules:['录音链路与监听建立','麦克风选择与摆位','人声录制与分轨管理','现场沟通与素材整理']},mixing:{name:'混音课程',intro:'理解频段、动态、空间和总线处理，让作品获得清晰、均衡的完成度。',icon:'sliders-horizontal',modules:['EQ 与频段整理','压缩与动态塑形','混响、延迟与声场','混音检查与成品准备']},arrangement:{name:'编曲课程',intro:'从歌曲结构和和声骨架出发，完成节奏、音色与配器的制作表达。',icon:'music-3',modules:['歌曲结构与参考分析','节奏骨架与低频设计','和声、音色与配器','编曲工程整理与交付']}}; const current=courses[course]; if(!current) return notFound(); return `<main class="shell page"><section class="page-title"><div class="eyebrow">ZHISHENG ACADEMY / ${course.toUpperCase()}</div><h1>${current.name}</h1><p>${current.intro}</p></section><section class="course-layout"><article class="course-overview"><span class="service-icon">${icon(current.icon)}</span><h2>课程方向</h2><p>面向音乐创作者、主播与音频爱好者，课程将专业流程拆解为可实践的学习模块。</p><button class="btn btn-primary" data-contact>咨询课程安排 ${icon('message-circle')}</button></article><section class="course-modules">${current.modules.map((module,index)=>`<article class="course-module"><span>${String(index + 1).padStart(2,'0')}</span><h3>${module}</h3><p>理论讲解、工程演示与可复用的制作方法。</p></article>`).join('')}</section></section></main>`; }
